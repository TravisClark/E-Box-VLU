  ...
  return (
    <div className={`relative `}>
      <svg
        ...
        width="24"
        height="24"
        className={`scale-75 cursor-pointer transition duration-500 ${changeBgColor ? 'fill-black' : 'fill-white'}`}
      ...
      {isShowingNotifications && (
        <div className={`w-96 h-30 bg-white absolute -right-28 mt-10 rounded-md flex flex-col ${classes.item}`}>
          <span className="font-medium p-4">Notifications</span>
          <ul className={`max-h-44 mb-10 overflow-hidden ${notifications.length > 0 && 'hover:overflow-y-scroll'}`}>
            {notifications.length > 0 ? notifications : <li className="text-black w-full text-center">You don't have any notifications</li>}
          </ul>
        </div>
      )}
    </div>
