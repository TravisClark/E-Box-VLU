...
export const Stars = ({ stars, id_question, refreshHandler }) => {
  ...
  return (
    <div className="rating flex space-x-2 border border-slate-300 h-fit py-1 px-2 justify-center rounded-md cursor-pointer group" onClick={onCheckHandler}>
      <svg
      ...
      <span className="h-full font-medium text-md mt-0.5">{stars.length}</span>
    </div>
  );
};
